...
const { getAnimationFilters, getDefaultFilters, sortOptions } = useFilters()

// SEO
usePageSeo('动漫', '热门动漫在线观看，日本动漫、国产动画、欧美动画等')

// State
const page = ref(1)
const pageSize = 24
const filterConfigs = getAnimationFilters()
const filters = ref<FilterOptions>(getDefaultFilters())

// 数据加载
const { data: listData, pending: loading, error, refresh } = useAsyncData(
  () => `cels-${filters.value.sort}-${filters.value.type}-${filters.value.year}-${filters.value.region}-${page.value}`,
  () => getMovieList('animation', filters.value, page.value, pageSize),
  {
    server: true,
    lazy: false,
